...
std::vector<at::Tensor> linear_relu_linear_forward(at::Tensor input, at::Tensor weight1, at::Tensor bias1, at::Tensor weight2, at::Tensor bias2) {
  ...
  auto relu_in = at::empty({batch_size, hidden_features}, input.type());
  auto output2 = at::empty({batch_size, out_features}, input.type());
  //auto reserved_space = at::empty({reserved_size}, inputs[0].type());
  // allocate fixed 4MB workspace for cublaslt for now, and this gets at least 4 MB
  auto lt_workspace = at::empty({1 << 22}, input.type());

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.type(), "linear_relu_linear_forward", [&] {
    ...
    auto result = linear_relu_linear_forward_cuda<scalar_t>(
        input.data_ptr<scalar_t>(),
        w1_ptr,
        b1_ptr,
	w2_ptr,
	...
	hidden_features,
        ...
        relu_in.data_ptr<scalar_t>(),
       // reserved_space.data_ptr<scalar_t>(),
        (void*) (lt_workspace.data_ptr<scalar_t>()));
  });

  return {output1, output2, relu_in};
}

std::vector<at::Tensor> linear_relu_linear_backward(at::Tensor input, at::Tensor relu_in, at::Tensor output1, at::Tensor weight1, at::Tensor weight2, at::Tensor d_output2) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.type(), "linear_bias_backward", [&] {
    //scalar_t* w_ptr = weight.data_ptr<scalar_t>();
    //scalar_t* d_b_ptr = d_bias.data_ptr<scalar_t>();
    auto result = linear_relu_linear_backward_cuda<scalar_t>(
        input.data_ptr<scalar_t>(),
        relu_in.data_ptr<scalar_t>(),
        output1.data_ptr<scalar_t>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("linear_relu_linear_forward", &linear_relu_linear_forward, "linear relu linear forward");
  m.def("linear_relu_linear_backward", &linear_relu_linear_backward, "linear relu linear backward");
}
